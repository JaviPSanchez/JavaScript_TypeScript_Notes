...
And polymorphism sounds a bit weird, which is because it comes from Greek, where it literally means "many shapes". Now, in the context of OOP, in simple terms, polymorphism means that a child class can overwrite a method that it inherited from a parent class.

And here are our user and admin classes again. But now we also have a third class, which is the author. Now admin and author are both really just special kinds of users, and so it makes sense that they both inherit from the user class, just like we studied in the last slide.

Therefore, they inherit all the properties and methods from the user class, but we're gonna focus on the login method now. Now let's say that an admin requires a different kind of login method. For example, a more secure one, which has two-factor authentication. And let's say that we also need a special login method for authors.
