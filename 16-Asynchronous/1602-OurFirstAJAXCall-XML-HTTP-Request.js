...
// /*
// So now that we know about AJAX and API's, let's actually make our first API call.So in this lecture, we're gonna build a nice UI component, which contains data about a certain country. And this data about the countries is actually gonna come from a third party online API

// Now in JavaScript, there are actually multiple ways of doing AJAX calls, But we're gonna start with the most old school one. And that's called the XML HTTP request function:
// */
// new XMLHttpRequest();
// //So we call this function and then all we need to do for now is to store the result in a new variable. Nos inventamos una, por ejemplo request:
// const request = new XMLHttpRequest();
// /*
// And so that's the first step of using the XML HTTP request way of doing AJAX calls.

// So first, I want you to know that XML HTTP requests exists, because you might actually need it in the future. And second, I want to show you how AJAX calls used to be handled with events and callback functions. And so only after that we should move on to a more modern way of handling asynchronous JavaScript, which is gonna be a feature called Promises.

// Next, we need the URL to which we will make the AJAX call.

// Cogemos el OBJECT que hemos creado y le pasamos el METHOD open() como primer argumento necesitamos pasar el tipo de request, and remember that the type of HTTP request to get data is simply called GET. And second, we need a string containing the URL to which the AJAX call should actually be made.

// La API seleccionada es la encontraremos en el gran repository GitHub de Public API's: https://github.com/JaviPSanchez/public-apis la API que vamos a utilizar se llama REST Countries: https://restcountries.eu/ dentro de la seccion de Geocoding. Podemos ver que esta API no necesita autentificacion, usa HTTPS, y muy importante comprobar el CORS, CORS stands for Cross Origin Resource Sharing.And without CORS, we cannot access a third party API from our own code.
...
// /*
// No funcionaria, And the reason why we cannot do this is because the result is simply not there yet, right. So this AJAX call that we just send off here, is being done in the background, while the rest of the code keeps running. And so this is the asynchronous, non-blocking behavior that we talked about in the last lecture.

// And instead, what we need to do is to register a callback on the request object for the load event. So request, and then again, our old friend, addEventListener:
// */
...
// /*
// So again, here, we basically send off the request with request.send() this request then fetches the data in the background. And so as soon as the data arrives, this callback function here will be called:
// */
...
/*
And by calling these functions here twice, we will basically have two AJAX calls happening at the same time. So in parallel, so to say. And if we reload this page here a couple of times, then they might appear in a different order. And the reason for that is basically that the data arrives at a slightly different time, each time that we're loading the page.

And so in fact, this really shows the non-blocking behavior in action. So as we call getCountryData here with 'spain', for the very first time, it sends of this request, and then JavaScript moves on in the code right away. And so it goes right here to this next line 'usa'.

And this, of course, fires off another AJAX call immediately, way before the data of 'spain' has actually arrived. So again, we will have two AJAX call happening at the same time. And so whatever one arrives first, will then fire the load event first. And so if the first one is the AJAX call for the 'france',
then the first element that's gonna be printed will be 'france'. And only after that the rest of data arrives.

Now, if we actually wanted these requests to be made in a specific, like predefined order, then we would basically have to chain the requests. Which means to make the second request only after the first request has finished. And that's actually what we're gonna do in the next lecture so that I can show you something that we developers called callback hell.
*/
