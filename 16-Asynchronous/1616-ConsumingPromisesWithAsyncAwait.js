...
And so in an ASYNC function like this one, we can use the await keyword to basically await for the result of this promise. So basically await will stop the code execution at this point of the function until the promise is fulfilled. And so until the data has been fetched in this case.

You might think isn't stopping the code, blocking the execution? Well, that's a really good question, but the answer is actually no, with async functions we are running asynchronously in the background. And so therefore it is not blocking the main threat of execution. So it's not blocking the call stack. And in fact, that's, what's so special about a single await. So it's the fact that it makes our code look like regular synchronous code while behind the scenes everything is in fact asynchronous.
...
So you see here that by using ASYNC/Await or asynchronous code here, really looks and feels like synchronous code. So we can simply await until the value of the promise is returned basically. And then just assign that value to a variable.
Something that was impossible before. So before we had to mess with callback functions and that was true in callback hell, but also by consuming promises with the then method. But now with ASYNC/Await, that is just completely gone.


Now, before you start using ASYNC/Await all over the place, you need to first understand that ASYNC/Await is in fact, simply syntactic sugar over the then method in promises. So of course behind the scenes, we are still using promises.
...
// console.log('First');
/*
And all without the chaining of promises like we had before. So this is really elegant. One more time, simply being able to essentially store the fulfilled promise value immediately into a variable without having to mess with callback functions.

But now let's actually finish the function here with all the other functionality as well. So with geolocation and also reverse geo-coding. So for that, we need to get our promise that we built before for geolocation,  So this one here:
*/
