...
However, there is one important thing missing. So right now, it might still be a little bit unclear what an async function actually is and how it works. And so let's now fix that. So to understand a bit better, what's actually happening here. Let's start by adding some more console.logs

And if we check the result, immediately we get the first log then the second one.
And of course only after that we get all the logs coming from the async function.


Now, if this was indeed a regular function and there would be a console.log in that regular function, then of course, that would appear here between one and two. But now let's say that we wanted to return some data from this function, all right?
...
However, we don't really want a new complete function here, and so instead we can use an IIFE. So remember IIFEs from way back, they are immediately-invoked function expressions. And remember that this is how we create one:
*/
...
/*
So we write function then here the function body, and then in the end we simply call it. And so of course we can also easily create an async IIFE as well, all right? And actually this pattern here is one of the last remaining cases for IIFEs, all right?


So let's start with the try catch block. So try catch, here We get access to the error and then let's actually immediately do this part. And then here we can simply await the whereAmI promise, okay? And then all we have to do is to store that result into the city variable and then log that to the console.


And we should get the exact same result and let's wait for it. And here we go.
So great. We managed to do the conversion and now everything is using async/await. And so that's much nicer. And now we know how to basically return data
from an async function and how to properly receive and handle that returned data. Right?
...
and returning values between them. And so that's the reason why I'm showing you all this. To make sure that you really correctly understand how async functions work behind the scenes.
*/
